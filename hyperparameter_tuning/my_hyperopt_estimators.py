...
#TODO: may be change eval metric which work by defaulting in the above function
# to 'auc' for xbg and lgmb. The change to lightGBM seem to be working but it 
# does not for xgb.

def instantiate_hyperopt_estimators(
    lightgbm_eval_metric=None,
    xgboost_eval_metric=None,
):
    """
    A function to instantiate the hyperopt estimators depending on the choice
    of the eval_metric.
    
    If no argument is passed, eval_metric for xgb and lgbm will default to 'auc'.    
    """    
    if lightgbm_eval_metric is None:
        lightgbm_eval_metric = 'auc'

    if xgboost_eval_metric is None:
        xgboost_eval_metric = 'auc'
    
    imbalance_ratio = 10 #(N_neg / N_positive) for xgboost

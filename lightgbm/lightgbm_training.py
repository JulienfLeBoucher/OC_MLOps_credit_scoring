...
hyperopt_estimators = [
    HyperoptEstimator(
        ...
        space={
                # 'boosting_type': hp.choice('boosting_type', ['goss']),
                'num_leaves': hp.uniformint('num_leaves', 50, 70),
                'max_depth': hp.uniformint('max_depth', 10, 18),
                'learning_rate': hp.loguniform('learning_rate', log(0.005), log(0.04)),
                'subsample_for_bin': scope.int(hp.quniform('subsample_for_bin', 120000, 300000, 20000)),
                'min_child_samples': scope.int(hp.quniform('min_child_samples', 150, 300, 10)),
                'reg_alpha': hp.uniform('reg_alpha', 0.8, 1.0),
                'reg_lambda': hp.uniform('reg_lambda', 0.55, 1.0),
                'colsample_bytree': hp.uniform('colsample_by_tree', 0.7, 1.0),
                'is_unbalance': hp.choice('is_unbalance', [True, False]),
                'min_split_gain': hp.uniform('min_split_gain', 0.02, 0.05),
